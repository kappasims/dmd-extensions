...
depth that isn't listed is converted to `Rgb24`, `Gray4` or `Gray2`, the first of those that
`messages` lists, with the color or palette set on the destination. Nothing converts to `Gray8`, so
a section that lists only `gray8` gets only `Gray8` frames.
